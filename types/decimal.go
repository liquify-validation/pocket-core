...
func (d BigDec) FracPow(power BigDec, denominator int64) BigDec {
	...
	//Multiply out B/C to isolate B and truncate B to power to denominator
	Buntruncated := NewInt(power.Mul(NewDec(denominator)).RoundInt64())
	B := Buntruncated.Sub(Buntruncated.Sub(NewInt(denominator))).ToDec()
